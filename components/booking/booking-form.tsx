...
// APIから特別料金データをロードし、PricingDisplayコンポーネントへ渡します。
...
export function BookingForm() {
  ...
  useEffect(() => {
    fetch('/api/rates')
      .then(res => res.json())
      .then(data => { if(data.rates) setSpecialRates(data.rates) })
      .catch(err => console.error('Rates load failed', err))
  }, [])
  ...
  const numberOfGuests = parseInt(numberOfGuestsVal, 10) || 0
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('予約作成失敗')
      setIsSuccess(true)
      toast({ title: "予約リクエスト完了", description: "メールにて決済リンクをお送りします。" })
    } catch (error) {
  ...
  if (isSuccess) {
    return (
      <div className="text-center p-8 bg-card rounded-lg border shadow-sm">
        ...
        <p className="text-muted-foreground mb-6">確認メールをお待ちください。</p>
        <Button onClick={() => window.location.reload()}>新しい予約</Button>
      </div>
    )
  }
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      <div>
        ...
        />
        {checkIn && checkOut && (
          <PricingDisplay checkIn={checkIn} checkOut={checkOut} guests={numberOfGuests} specialRates={specialRates} />
        )}
      </div>
      <div>

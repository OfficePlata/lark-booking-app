...
type Message = {
  id: number, sender: 'guest' | 'owner', name: string, message: string, timestamp: string, type: 'mail'
}

const MOCK_MESSAGES: Message[] = [
  { id: 1, sender: 'guest', name: '山田 太郎', message: 'はじめまして。チェックイン時間の変更は可能ですか？', timestamp: '10:30', type: 'mail' },
  { id: 2, sender: 'owner', name: 'STAY YOKABAN', message: 'お問い合わせありがとうございます。何時ごろをご希望でしょうか？', timestamp: '10:45', type: 'mail' },
]
    ...
    <div className="flex flex-col h-[600px] w-full max-w-md bg-background border rounded-xl shadow-xl overflow-hidden mx-auto">
      <div className="bg-primary/5 p-4 border-b flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Avatar><AvatarImage src="/placeholder-user.jpg" /><AvatarFallback>G</AvatarFallback></Avatar>
            ...
            <div key={msg.id} className={`flex ${msg.sender === 'owner' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex gap-2 max-w-[85%] ${msg.sender === 'owner' ? 'flex-row-reverse' : ''}`}>
                <Avatar className="w-8 h-8 mt-1">
                  {msg.sender === 'guest' ? <AvatarFallback>G</AvatarFallback> : <AvatarFallback className="bg-primary text-primary-foreground">Me</AvatarFallback>}
                </Avatar>
      ...
      <div className="p-4 bg-background border-t">
        <div className="flex gap-2 items-end">
          <Input placeholder="返信内容..." className="min-h-[50px] py-3 resize-none" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleSend()} />

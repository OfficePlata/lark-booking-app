...
const MOCK_MESSAGES: Message[] = [
  { 
    ...
    sender: 'guest', 
    name: '山田 太郎', 
    message: 'はじめまして。チェックインについて質問があります。', 
    timestamp: '10:30', 
    type: 'mail' 
  },
  { 
    ...
    <div className="flex flex-col h-[600px] w-full max-w-md bg-background border rounded-xl shadow-xl overflow-hidden">
      ...
      <div className="bg-primary/5 p-4 border-b flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">メールチャットモード</p>
          </div>
        </div>
              ...
              <div className={`flex gap-2 max-w-[85%] ${msg.sender === 'owner' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className={`text-[10px] text-muted-foreground ${msg.sender === 'owner' ? 'text-right' : 'text-left'}`}>
                    {msg.type === 'mail' && '✉️ メール経由 • '} {msg.timestamp}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="p-4 bg-background border-t">
        <div className="flex gap-2 items-end">
          <Input 
            placeholder="返信メールを入力..." 
            className="min-h-[50px] py-3 resize-none"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleSend()}
          />
          <Button size="icon" className="h-[50px] w-[50px] shrink-0" onClick={handleSend}>
            <Send className="w-5 h-5" />
          </Button>
        </div>
        <p className="text-[10px] text-muted-foreground mt-2 text-center">
          ※ここで送信すると、相手にはメールとして届きます
        </p>
      </div>
    </div>
  )
